...
void get_lexeme(const Lexeme *lexeme, double *operand_out, char *action_out);
...
void set_lexeme(Lexeme *lexeme, double operand, char action, int type) {
    lexeme->actual_type = type;
...
void get_lexeme(const Lexeme *lexeme, double *operand_out, char *action_out) {
    ...
    *action_out = '\0';
    ...
    //  expecting double (number, operand)
    if (lexeme->actual_type == LT_OPERAND) {
        // *operand_out = lexeme->data;
        // return;
        
        const double *const pointer_to_double = (char*)lexeme->data;
        *operand_out = *pointer_to_double;
        return;
    } 
    
    const double *const pointer_to_double = &(lexeme->data);
    const char *const pointer_to_char = (char*)pointer_to_double;
...
void visual_test_get_lexeme(void get_lexeme(const Lexeme*, double *, char *)){
    Lexeme lexemes[33];
    set_lexeme(lexemes + 0, 56.0, '\0', LT_OPERAND);
    set_lexeme(lexemes + 1, 0.0, '-', LT_ACTION);
